...
describe('檢查輸贏報表', () => {
    test.only('點擊輸贏報表', async () => {
        const xpathForsearch = "//*[text()='統計報表']";
        ...
        await new Promise(resolve => setTimeout(resolve, 1000));
        const Winlosereport = await page.$eval('#root > div > div > div > div > main > div > div:nth-child(1) > div > div > h2', element => element.textContent.trim());
        expect(Winlosereport).toBeTruthy();
        assert.equal(Winlosereport, '輸贏報表', 'text is incorrect');
    })
    test.only('檢查麵包屑', async () => {
        ...
        assert.equal(Breadcrumbs, '總覽/統計報表/輸贏報表', 'text is incorrect');
